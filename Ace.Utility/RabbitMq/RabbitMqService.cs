    ...
    public class RabbitMqService : IDisposable
    {
        ...
        private static RabbitMqAttribute GetRabbitMqAttribute<T>()
        {
            if (_rabbitMqAttribute == null)
            {
                ...
                var attribute = typeOfT.GetCustomAttributes(typeof(RabbitMqAttribute), false).FirstOrDefault();
                if (attribute == null) return null;
                _rabbitMqAttribute = (RabbitMqAttribute)attribute;
            }
        ...
        private void PublishToDead<T>(string queue, string body, Exception ex) where T : class
        {
            ...
            var deadLetterQueue = queueInfo.QueueName;
        ...
        /// <param name="isProperties">是否持久化</param>
        /// <param name="handler">消费处理</param>
        /// <param name="isDeadLetter">是否是死信队列</param>
        public void Subscribe<T>(string queue, bool isProperties, Action<T> handler, bool isDeadLetter) where T : class
        {
            ...
            consumer.Received += (model, ea) =>
            {
                ...
                {
                    Console.WriteLine("RabbitMq队列接收消息{0}", ex.InnerException.Message);
                    if (!isDeadLetter)
                        PublishToDead<DeadLetterQueue>(queue, body, ex);
                }
                finally
                {
